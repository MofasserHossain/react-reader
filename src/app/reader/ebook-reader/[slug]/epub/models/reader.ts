...
import { Rendition as Ren } from '@/types/epub/radiation'
...
export type Renditions = Omit<Rendition, 'getContents'> &
  Pick<Ren, 'getContents'>
...
export type TabParam = ConstructorParameters<typeof BookTab | typeof PageTab>[0]
...
export class Group {
  ...
  addTab(param: TabParam | Tab) {
    ...
    console.log(`\n\n param:`, param, index, isPage, isTab, tab)
    return tab
  }
...
export class Reader {
  ...
  resize() {
    this.groups.forEach(({ bookTabs }) => {
      bookTabs.forEach(({ rendition }: any) => {
        try {
          rendition?.resize()
        } catch (error) {
          console.error(error)
